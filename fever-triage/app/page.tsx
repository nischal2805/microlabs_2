...
export default function Home() {
	...
	const [currentStep, setCurrentStep] = useState<
		"form" | "results" | "dashboard" | "findDoctors" | "analytics"
	>("form");
	const [loading, setLoading] = useState(false);
	const [results, setResults] = useState<ComprehensiveTriageResponse | null>(
		null
	);
		...
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(
				...
				(error) => {
					...
					setLocationError(
						"Location access not available - continuing without location context"
					);
				},
				{
	...
	const handleFormSubmit = async (patientData: PatientData, photo?: File) => {
		...
		try {
			...
			if (err instanceof APIError) {
				// Check if it's a photo analysis error
				if (
					err.message.includes("Photo analysis") ||
					err.message.includes("not configured")
				) {
					setError(
						`Assessment failed: ${err.message}. Try submitting without a photo or check backend configuration.`
					);
				} else {
							...
							<header className="text-center mb-8">
								...
								<div className="flex justify-end mb-4">
									<div className="flex items-center space-x-4">
										{(authUserProfile || user?.displayName) && (
								...
								<p className="text-lg text-blue-700 max-w-2xl mx-auto leading-relaxed">
									Intelligent clinical decision support powered by AI. Get
									instant fever triage assessments based on clinical protocols
								...
								<Chatbot
									assessmentContext={
										results
											? `Recent assessment: ${
													results.severity
											  } severity - ${results.recommended_action.slice(
													0,
													100
											  )}...`
											: undefined
							...
							<footer className="text-center mt-12 text-gray-600">
								<div className="max-w-2xl mx-auto">
									<p className="text-sm mb-4">
										Powered by AI • Built for healthcare professionals and
										educational use
									</p>
									<div className="flex justify-center space-x-6 text-xs flex-wrap gap-2">
