...
export default function FindDoctors({ googleMapsLoaded }: FindDoctorsProps) {
					...
					<div className="flex items-center">
						...
						<div>
							...
							<p className="text-sm text-yellow-700 mt-1">
								Google Maps is loading. Please wait a moment and try again.
							</p>
						</div>
					</div>
				</div>
	...
	const handleSearchDoctors = async () => {
		...
		if (!googleMapsLoaded) {
			setError(
				"Google Maps is not loaded. Please check your API configuration."
			);
			return;
		}
		...
		try {
			...
			const searchKeyword =
				isSmartSearch && feverAnalysis?.searchKeywords
					? feverAnalysis.searchKeywords
					: undefined;

			const nearbyDoctors = await PlacesService.findNearbyDoctors(
				userLocation,
				searchKeyword
			);
			...
			if (nearbyDoctors.length === 0) {
				setError(
					"No doctors found in your area. Try expanding the search radius."
				);
				setDoctors([]);
				return;
			}
			...
			if (isSmartSearch && feverAnalysis?.recommendedSpecialties) {
				console.log(
					"Applying smart filtering for specialties:",
			...
			const doctorsWithDirections =
				await PlacesService.getDirectionsForMultiplePlaces(
					userLocation,
					filteredDoctors
				);
			...
			doctorsWithDirections.sort((a, b) => {
				const aDistance = a.distance
					? parseInt(a.distance.split(" ")[0])
					: Infinity;
				const bDistance = b.distance
					? parseInt(b.distance.split(" ")[0])
					: Infinity;
			...
			if (errorMessage.includes("ZERO_RESULTS")) {
				setError(
					"No doctors found in your area. Try searching from a different location."
				);
		...
		<div className="max-w-4xl mx-auto p-6 space-y-6">
			<div className="text-center space-y-2">
				...
				<p className="text-gray-600">
					{isSmartSearch
						? `Finding specialized doctors for ${
								feverAnalysis?.feverType?.replace("_", " ") || "your condition"
						  }`
				...
				{isSmartSearch && feverAnalysis && (
					<div className="bg-green-50 border border-green-200 rounded-lg p-3 inline-block">
						<p className="text-sm text-green-800">
							Recommended:{" "}
							{feverAnalysis.recommendedSpecialties[0]?.specialty?.replace(
								"_",
								" "
